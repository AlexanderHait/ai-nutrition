...
const ERRORS: Record<string, string> = {
  ...
  processing: "ЮKassa ещё обрабатывает запрос. Повторный переход восстановит тот же платёж и не создаст второй.",
...
export default async function Page({
  ...
  return (
    <>
      ...
      <div className="pageHead"><div><p>TeddY / Оплата</p><h1>{product.title}</h1><span>{product.description}</span></div></div>
      <section className="card top" style={{ maxWidth: 720 }}>
        <div className="sectionTitleRow"><div><h2>{price} ₽</h2><span className="muted">Доступ на {product.period_days} дней · без автоматического списания</span></div><CreditCard /></div>
        ...
        <form action="/api/payments/yookassa/create" method="post" className="checkinForm top">
          ...
          <label>Электронная почта для чека<input type="email" name="email" autoComplete="email" inputMode="email" defaultValue={current.email || ""} placeholder="name@example.com" required /></label>
