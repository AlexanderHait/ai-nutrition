...
function activityLabel(value: unknown) {
  ...
  return labels[String(value || "")] || "Не указана";
}
...
export default async function Page({
  ...
  const protein = Number(data.settings?.protein_target || 0);
  ...
  return (
    <>
      ...
      {query.telegram === "linked" ? <div className="successNotice"><CheckCircle2 size={17} />Telegram привязан к аккаунту.</div> : null}
      ...
      <section className="profileOverview">
        <div className="profileOverviewItem"><span>Цель</span><b>{goalKind(data.settings?.goal)}</b></div>
      ...
      <div className="profileLayout top">
        <details className="secondaryDisclosure profileEditor">
          <summary>
            <span><b>Изменить параметры питания</b><small>Цель, рост, активность, калории и КБЖУ</small></span>
            <ChevronDown size={18} />
          </summary>
          <div className="disclosureBody">
            <form action="/api/client/settings" method="post" className="form profileForm modern">
              <div className="formSection">
                <div className="formSectionTitle">Основные данные</div>
                ...
                <div className="grid2">
                  <label>Пол<select name="sex" defaultValue={data.settings?.sex || ""}><option value="">Не указан</option><option value="male">Мужской</option><option value="female">Женский</option></select></label>
                  <label>Возраст<input name="age_years" type="number" min="14" max="100" defaultValue={data.settings?.age_years || ""} /></label>
                </div>
                <div className="grid2">
                  <label>Дата рождения<input name="birth_date" type="date" defaultValue={data.settings?.birth_date || ""} /></label>
                  <label>Активность<select name="activity_level" defaultValue={data.settings?.activity_level || ""}><option value="">Не указана</option><option value="low">Низкая</option><option value="light">Лёгкая</option><option value="moderate">Средняя</option><option value="high">Высокая</option><option value="very_high">Очень высокая</option></select></label>
                </div>
                <div className="grid2">
                  <label>Рост, см<input name="height_cm" type="number" min="100" max="250" step="0.1" defaultValue={data.settings?.height_cm || ""} /></label>
                  <label>Целевые калории<input name="kcal_target" type="number" min="800" max="7000" defaultValue={kcal} /></label>
                </div>
              </div>
              ...
              <div className="formSection">
                ...
                <div className="grid2">
                  <label>Текущий вес, кг<input name="current_weight_kg" type="number" min="30" max="400" step="0.1" defaultValue={currentWeight || ""} /></label>
                  <label>Целевой вес, кг<input name="target_weight_kg" type="number" min="30" max="400" step="0.1" defaultValue={targetWeight || ""} /></label>
                </div>
              </div>
              ...
              <div className="formSection">
                ...
                <div className="macroInputs">
                  <label>Белки, г<input name="protein_target" type="number" min="0" max="500" step="0.1" defaultValue={protein || ""} /></label>
                  <label>Жиры, г<input name="fat_target" type="number" min="0" max="500" step="0.1" defaultValue={fat || ""} /></label>
                  <label>Углеводы, г<input name="carb_target" type="number" min="0" max="1000" step="0.1" defaultValue={carb || ""} /></label>
                </div>
              </div>
        ...
        <aside className="profileAside" id="weight">
          <section className="card">
            ...
            {currentWeight > 0 && targetWeight > 0 ? (
              <div className="weightTarget">Сейчас {fmt(currentWeight, 1)} кг · цель {fmt(targetWeight, 1)} кг · осталось {fmt(Math.abs(currentWeight - targetWeight), 1)} кг</div>
            ) : (
              <p className="muted">Добавь текущий и целевой вес — появится понятный ориентир.</p>
            )}
          </section>
        </aside>
      </div>
