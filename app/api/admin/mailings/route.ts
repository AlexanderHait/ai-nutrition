...
function go(req:Request,path:string){
  // POST form submissions must become GET after redirect.
  // 307/308 preserve POST and can make Next.js treat the page as a missing Server Action.
...
async function ensureMediaBucket(){
  ...
  const created=await s.storage.createBucket(BUCKET,{
    public:false,
    fileSizeLimit:MAX_FILE,
    allowedMimeTypes:ALLOWED,
  });
  if(created.error&&!String(created.error.message||"").toLowerCase().includes("already")){
    throw new Error(`Storage bucket: ${created.error.message}`);
  }
}
...
async function recipients(segment:string){
  ...
  if(segment==="inactive3"||segment==="inactive7"){
    ...
    for(const x of meals||[]){
      const id=Number(x.chat_id),ts=new Date(x.eaten_at).getTime();
      last.set(id,Math.max(last.get(id)||0,ts));
    }
    for(const x of logs||[]){
      const id=Number(x.chat_id),ts=new Date(x.created_at).getTime();
      last.set(id,Math.max(last.get(id)||0,ts));
    }
...
async function signedMedia(path:string|null){
  if(!path)return null;
  const s=getSupabaseAdmin();
  const {data,error}=await s.storage.from(BUCKET).createSignedUrl(path,60*60);
...
export async function POST(req:Request){
  ...
  try{
    ...
    if(attachment instanceof File&&attachment.size>0){
      if(!ALLOWED.includes(attachment.type)||attachment.size>MAX_FILE)
        return go(req,"/admin/mailings?error=attachment");
      ...
      const {error}=await s.storage.from(BUCKET).upload(
        ...
        {contentType:attachment.type,upsert:false,cacheControl:"3600"},
      );
    ...
    if(action==="send"){
      ...
      const {error}=await s.from("mailings").insert({
        title,content,segment,status:"sent",
        recipient_count:ids.length,sent_count:result.sent,
        sent_at:new Date().toISOString(),
        media_path:mediaPath,media_kind:mediaKind,
      });
      if(error)throw new Error(`Mailing save: ${error.message}`);
      return go(req,`/admin/mailings?ok=1&sent=${result.sent}&failed=${result.failed}`);
    }

    const {error}=await s.from("mailings").insert({
      title,content,segment,status:"scheduled",
