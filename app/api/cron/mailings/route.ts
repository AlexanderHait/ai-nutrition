...
export async function GET(req:Request){
 ...
 for(const m of data||[]){
  ...
  let sent=0;for(const chat_id of ids){const method=photo?"sendPhoto":"sendMessage",body=photo?{chat_id,photo,caption:m.content}:{chat_id,text:m.content};const r=await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/${method}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});if(r.ok)sent++}
