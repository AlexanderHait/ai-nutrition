...
export default async function Page({searchParams}:{searchParams:SP}){
 const q=await searchParams;const {profiles,meals,logs,settings,subscriptions}=await adminClientsData();const today=dayKey();
 ...
 const rows=profiles.map(p=>{const id=Number(p.telegram_id),pm=mealsByClient.get(id)||[],todayMeals=pm.filter(m=>mealDay(m)===today),weekDays=new Set(pm.map(mealDay));const last=[lastMeal.get(id),lastLog.get(id)].filter(Boolean).sort().reverse()[0] as string|undefined;return{p,id,st:settingMap.get(id),sub:subMap.get(id),today:sumMeals(todayMeals),todaySessions:mealSessions(todayMeals).length,activeDays:weekDays.size,last}})
