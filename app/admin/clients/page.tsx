...
export default async function Page({searchParams}:{searchParams:SP}){
 ...
 return <>
  <div className="pageHead"><div><p>AI‑Nutrition / Клиенты</p><h1>Клиенты</h1><span>Тариф, роль, КБЖУ и активность клиента в одном списке. Статусы совпадают с production-таблицами бота.</span></div></div>
  ...
  <section className="card tableCard clientsTable optimizedClients"><div className="tableHead clientsGridV4"><span>Клиент</span><span>Статус</span><span>Цель / КБЖУ</span><span>Сегодня</span><span>7 дней</span><span>Активность</span></div>{rows.map(x=><Link href={`/admin/clients/${x.id}`} className="tableRow clientsGridV4 clientRowV2" key={x.p.id}><div className="clientIdentity"><TelegramAvatar profile={x.p} size="small"/><div><b>{x.p.first_name||"Без имени"}</b><small>{x.p.username?`@${x.p.username}`:`ID ${x.id}`}</small></div></div><div className="clientStatusStack"><span className={`planPill ${x.plan}`}>{planLabel(x.plan)}</span>{x.isAdmin&&<AdminBadge/>}</div><div className="clientGoalMacro"><b>{goalKind(x.st?.goal)}</b><small>{macroTarget(x.st)}</small></div><div className="clientMetric"><b>{fmt(x.today.kcal)} ккал</b><small>{x.todaySessions} приём.</small></div><div className="clientMetric"><b>{x.activeDays}/7</b><small>дней с рационом</small></div><div className={`clientActivity ${x.last&&Date.now()-new Date(x.last).getTime()>3*86400000?"stale":""}`}>{relativeActivity(x.last)}</div></Link>)}</section>
 </>;
}
