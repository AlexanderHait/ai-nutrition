...
2. **Задвоение позиций** — разобрано. Это было не задвоение: две строки «Пирожок» по 79 ₽ и 61 ₽ — разные товары, ошибкой было одинаковое название на обеих. Закрыто узлом `Verify Receipt Line Integrity`. Остаётся приятное продолжение: когда две строки чека действительно одинаковы (та же цена), показывать одну строку с количеством ×2 вместо двух.
