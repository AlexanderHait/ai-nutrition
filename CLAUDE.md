...
2. **Задвоение позиций** — «Пирожок Вишневый» приходит двумя строками, коротким и длинным названием. Вероятная причина: берётся из веб-поиска, а не из каталога.
3. **Запись в каталог с фото.** Узел `Build Trusted Cache Rows` → `UPSERT Trusted Food Cache Supabase`. Проверить, не отдаёт ли он пустой список, как было в текстовом сценарии. Правка по тому же образцу. Закроет пункт 2 и наполнение каталога.
...
1. `Build Trusted Cache Rows` — запись в каталог с фото (самое дешёвое)
2. Подбор по цене позиции против выдумок зрения
